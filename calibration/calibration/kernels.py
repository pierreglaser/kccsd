...
class ApproxMedianHeuristicKernel(
    kernel_on_models[BM, MedianHeuristicKernel[BM2], BM2],
):
    sigma: float
    ...
    def with_approximate_median_heuristic(
        ...
        new_sigma = approx_k.with_median_heuristic(Xs, indices).sigma
        return self.replace(sigma=new_sigma)
...
class DiscreteExpMMDKernel(
    ...
    def with_median_heuristic(
        self, X: DiscretizedModel[Any], indices: Optional[Tuple[Array, Array]] = None
    ) -> Self:
        ...
        if isinstance(kernel, MedianHeuristicKernel):
            ...
            # if possible.
            sigma = median_median_heuristic_discrete(kernel, X, indices)
            kernel = kernel.replace(sigma=sigma)
        return super(
            DiscreteExpMMDKernel, self.replace(ground_space_kernel=kernel)
        ).with_median_heuristic(X, indices)
...
class ExpMMDGaussianKernel(
    ...
    def with_median_heuristic(
        self, X: GaussianConditionalModel, indices: Optional[Tuple[Array, Array]] = None
    ) -> Self:
        # We also apply the median heuristic to the ground-space kernel.
        sigma = median_median_heuristic_euclidean_gaussian(X, indices)
